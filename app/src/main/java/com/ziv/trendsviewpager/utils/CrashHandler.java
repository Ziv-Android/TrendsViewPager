...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	// 用来存储设备信息和异常信息
	private Map<String, String> mAppInfos = new HashMap<String, String>();
	
	private static String sCrashHandlerPath = "/sdcard/crash/";
...
//				mContext.startActivity(intent);
	...
	public void collectDeviceInfo(Context ctx) {
		...
		for (Field field : fields) {
			try {
				...
				mAppInfos.put(field.getName(), field.get(null).toString());
	...
	private String saveCrashInfo2File(Throwable ex) {
		...
		try {
			...
			String fileName = AppInfoUtil.getPackageInfo(mContext).packageName + "_crash-" + time + "-" + timestamp + ".txt";
			if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
				FileUtil.createDirectory(sCrashHandlerPath);
				FileOutputStream fos = new FileOutputStream(sCrashHandlerPath + fileName);
